...
def test_urls_unblock(client):
    ...
    assert response.json() == {"https://example.com/x": False}
...
def test_urls_all(client):
    ...
    assert set(response.json()) == {"https://example.com/x", "https://example.com/y"}
...
def test_urls_stats(client):
    ...
    assert response.json() == {"total_urls": 3, "unique_domains": 2}
...
def test_urls_block_returns_import_counts(client):
    ...
    assert response.json() == {"received": 2, "added": 1, "merged": 1}
...
def test_urls_block_unescapes_html_entities(client):
    ...
    response = client.get("/urls/all")
    assert "https://example.com/?a=1&b=2" in response.json()


# Input validation ---------------------------------------------------------------------------------

def test_settings_get_wrong_content_type_returns_error(client):
    response = client.post("/settings/get", content="not json", headers={"Content-Type": "text/plain"})
